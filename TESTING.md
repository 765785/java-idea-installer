...
使用全新的 Sandbox，在沙盒内关闭代理，只打开公开网页，不提供额外教程。测试者只能按照网页文字完成：

1. 点击下载插件时确认页面说明 BAT 只是启动器，后续仍会下载约 `1.1 GB`。
2. 双击 BAT 后如出现 SmartScreen，按网页提示选择“更多信息”再点“仍要运行”。
3. 安装盘列表出现时，分别验证输入 `1`、`C` 和直接回车都选择 C 盘。
4. 下载期间确认终端显示来源、文件大小、进度条和“请勿关闭窗口”。
5. UAC 出现时按网页提示点击“是”。
6. 完成后确认终端显示成功信息，IDEA 自动打开。
7. 关闭 IDEA 后再次运行 BAT，确认自动复用且不下载、不触发 UAC。

记录每一步耗时和停顿。任何需要搜索教程、猜测英文提示或手动执行命令的情况都视为零基础流程失败。
