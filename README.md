...
- 📈 安装时自动打开本地进度页，每两秒轮询 `progress.json`。
- 🧪 安装完成后自动编译并运行 `Hello World`。
- 🛠️ 支持 `JAVA_HOME`、`PATH`、多 JDK、IDEA 损坏、Gatekeeper、VC++ Runtime 和端口问题修复。
...
1. 打开公开网页，确认顶部识别到的系统。
2. 下载对应工具包并解压。
3. Windows 双击 `detect.bat`；macOS 双击 `detect.command`。
4. 按网页中的检测报告下载安装工具包，Windows 运行 `install.bat`，macOS 运行 `install.command`。
5. 安装完成后点击本地进度页中的“返回网页查看最终报告”。
...
浏览器安全模型不允许网页读取完整本机软件列表或静默运行安装程序。本项目的脚本只在你下载并确认后执行。
